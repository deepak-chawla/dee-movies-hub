...
export default function ItemModal({ children, id, media }) {
  ...
  return (
    <>
      <div className="card" style={{cursor: "pointer"}} onClick={handleOpen}>
        {children}
      </div>
      <Modal
      ...
      >
        <Fade in={open}>{content && (
          <div className={classes.paper}>
          <div className="content-modal">
            {<img  alt={content.name || content.title} src={content.poster_path ? 
            `${img_500}/${content.poster_path}`
            : unavailable } className='potrait'/>}
            {
              <img  alt={content.name || content.title} src={content.poster_path ? 
